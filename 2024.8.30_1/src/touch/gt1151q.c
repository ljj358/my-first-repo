...
int GT1151_WR_Reg(int reg,unsigned char *buf, int len)
{
	...
	if(ret < 0)
    {
        rt_kprintf("write_bytes_reg error!!!\r\n");
        return -1;
    }
...
int GT1151_RD_Reg(int reg,unsigned char *buf,int len)
{
    ...
    if(ret < 0)
    {
        rt_kprintf("read_bytes_Data error!!!\r\n");
        return -1;
    }
...
void check_sum(void)
{
	...
	checksumL=checksum;
	rt_kprintf("chksum:0x%X,\r\n",checksum);
	rt_kprintf("chksumH:0x%X,\r\n",checksumH);
	rt_kprintf("chksumL:0x%X,\r\n",checksumL);
}
...
unsigned char GT1151_Init(void)
{
	...
	rt_kprintf("IC FW:0x%X\r\n",(temp[0]<<8|temp[1]));
	GT1151_RD_Reg(GT_CFGS_REG,temp,1);//读取GT_CFGS_REG寄存器
	rt_kprintf("Default Ver:0x%x\r\n",temp[0]);
	GT1151_RD_Reg(GT_PID_REG,temp,4);//读取产品ID
	rt_kprintf("CTP ID:GT%s\r\n",temp);	//打印ID
	check_sum();
	//rt_kprintf("Default Ver:0x%x\r\n",temp[0]);
	GT1151_Send_Cfg(1);

	gpio_write(GT_INT,1);//必须配置成输出模式
	rt_thread_delay(5);
	gpio_write(GT_INT,0);
	...
	if(strcmp((char*)temp,"1158")==0)//ID==1151
	{
		...
		rt_kprintf("Config Info:\r\n");
		for( i = 0; i < 239; i++ )
		{
		rt_kprintf("0x%02X,",Cfg_Info[i]);
		if((i+1)%10==0)
		rt_kprintf("\r\n");
		}
		rt_kprintf("\r\n");
