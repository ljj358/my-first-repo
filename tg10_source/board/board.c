...
static void board_stage_mark(rt_uint32_t stage)
{
    LOG_D("board stage %u", stage);
}
...
//	rt_memheap_init(&tcmsram_heap, "sram2", (void *)STM32_SRAM1_START, (rt_size_t)(STM32_SRAM1_END - STM32_SRAM1_START));
	rt_system_sram1_init((void *)STM32_SRAM1_START, (void *)STM32_SRAM1_END);
	
	rt_system_sram3_init((void *)STM32_SRAM3_START, (void *)STM32_SRAM3_END);
	
	rt_system_sram4_init((void *)STM32_SRAM4_START, (void *)STM32_SRAM4_END);
    ...
    /* Keep USB1 enabled before RT-Thread device init runs, otherwise the
     * usbd/vcom stack may come up while the external USB switch is still off. */
